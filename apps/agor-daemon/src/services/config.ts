  ...
  async patch(_id: null, data: Partial<AgorConfig>, _params?: Params): Promise<AgorConfig> {
    ...
    if (data.ide) {
      ...
      if (data.ide.vscode) {
        ...
        const { enabled, preferred_mode, remote, tunnel } = data.ide.vscode;
        ...
        if (tunnel) {
          config.ide.vscode.tunnel = config.ide.vscode.tunnel || {};
          if (tunnel.name !== undefined) config.ide.vscode.tunnel.name = tunnel.name;
          if (tunnel.displayName !== undefined) {
            config.ide.vscode.tunnel.displayName = tunnel.displayName;
          }
        }
      }
